...
export interface ClientOptions {
   ...
   * cookie rides along without asking. A native app is always cross-origin
   * and has to opt in with 'include'.
   */
  credentials?: RequestCredentials
}

export function createGraphQLClient({ baseUrl, credentials = 'same-origin' }: ClientOptions): Client {
  return new Client({
    url: `${baseUrl}/graphql`,
...
export async function uploadImage(
  { baseUrl, credentials = 'same-origin' }: ClientOptions,
